...
Citizen.CreateThread(function()
	...
	Keybinds.Add("Weapons", "+switchWeaponMode", "-switchWeaponMode", "Switch Weapon Mode")
end)
...
function SwitchWeaponMode()
	...
	if AllowedModes[pWeaponHash] == nil then
		...
		TriggerEvent("cl_SendNotify", "You have no modes to cycle through", 2)
		return
	end
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			TriggerEvent("cl_SendNotify", "You can't change mode from: " .. FireRateTypes[SELECTED_FIRE_RATE].label , 2)
		else
			SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
			TriggerEvent("comet-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
	...
	TriggerEvent("cl_SendNotify", "Changed fire mode: " .. selectedRate.label , 1)
	TriggerEvent("comet-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end
...
RegisterNetEvent("comet-weapons:client:weaponEquiped")
AddEventHandler("comet-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("comet-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("comet-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("comet-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
