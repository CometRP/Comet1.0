...
local function openMenu(data)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			local img = "comet-inventory/nui/"
			if exports['comet-inventory']:itemListInfo(tostring(v["icon"])) then
				if not string.find(exports['comet-inventory']:itemListInfo(tostring(v["icon"])).image, "icons/") then
					img = img.."icons/"
				end
				v["icon"] = img..exports['comet-inventory']:itemListInfo(tostring(v["icon"])).image
			end
		end
	end
...
RegisterNetEvent('comet-context:client:openMenu', function(data)
    openMenu(data)
end)

RegisterNetEvent('comet-contex:client:closeMenu', function()
    closeMenu()
end)
    ...
    if sendData then
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isBaseCommand then
                    TriggerServerEvent('comet-base:callCommand', data.params.event, data.params.args)
