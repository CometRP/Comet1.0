...
start = function() {
    ...
    setTimeout(function(){
        ...
        loadingIcon(300);   
        setTimeout(function(){
            $.post('https://comet-multicharacter/setupCharacters');
            setTimeout(function(){
                MultiCharacters.resetAll();
                $(".welcomescreen").fadeOut(150);
                $('.bars').fadeIn(1000)
                ...
                $.post('https://comet-multicharacter/removeBlur');
            }, 2000);
        }, 2000);
    }, 500);
    }, test && 1 || 1000);
...
$( "#streamermode" ).change(function() {
    ...
    $.post('https://comet-multicharacter/streamermode', JSON.stringify({val: valstreamer}));
});
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://comet-multicharacter/closeUI');
    $.post('https://comet-multicharacter/disconnectButton');
});
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            setupCharInfo('empty')
            $.post('https://comet-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
            clickDisable = true;
        } else {
...
$(document).on('click', '#create', function(e){
    e.preventDefault();
    $.post('https://comet-multicharacter/createNewCharacter', JSON.stringify({
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://comet-multicharacter/removeCharacter', JSON.stringify({
        cid: $(selectedChar).data("cid"),
    }));
