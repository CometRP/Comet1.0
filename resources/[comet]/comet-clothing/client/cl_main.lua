...
local statusS = false
RegisterNetEvent("comet-clothing:Spawning")
AddEventHandler("comet-clothing:Spawning", function(pStatus)
    statusS = pStatus
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if statusS == true then
            TriggerServerEvent("comet-spawn:inspawn", true)
        end
    end
end)
...
function OpenMenu(name, pPriceText, pPrice)
    ...
    if isAllowed then
        ...
        TriggerEvent("cl_SendNotify", "You are not welcome here!");
    end
end
...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("comet-spawn:finishedClothing","Finished")
        ...
        TriggerEvent("comet-spawn:finishedClothing","Old")
        LoadPed(oldPed)
    end
...
AddEventHandler("comet-base:initialSpawnModelLoaded", function()
...
RegisterNetEvent("comet-clothing:defaultReset")
AddEventHandler("comet-clothing:defaultReset", function()
    -- local LocalPlayer = exports["comet-base"]:FetchComponent("LocalPlayer")
...
AddEventHandler('comet-clothing:ListOutfits', function(skincheck)
  local menuData = {}
    ...
    -- TriggerEvent("cl_SendNotify", skincheck[i].slot .. " | " .. skincheck[i].name,i)
    menuData[#menuData + 1] = {
      title = skincheck[i].name,
      description = '',
      action = 'comet-ui:comet-clothing:changeOutfit',
      key = i,
    }
  end
  TriggerEvent('cl_DrawText',menuData)
end)
...
RegisterNetEvent('comet-clothing:openClothing')
AddEventHandler('comet-clothing:openClothing', function(pDontShowBarber, pShouldCost)
...
AddEventHandler("comet-base:polyzone:enter", function(zone, data)
    local currentZone = MenuData[zone]
    if currentZone then
        TriggerEvent('cl_DrawText',("[M] %s"):format(currentZone.text))
        TriggerEvent("comet-clothing:enable", true)
        listenForKeypress(zone, currentZone, ((data and data.isFree) and true or false))
    end
end)

AddEventHandler("comet-base:polyzone:exit", function(zone)
    local currentZone = MenuData[zone]
    if currentZone then
        listening = false
        TriggerEvent('cl_HideText')
        TriggerEvent("comet-clothing:enable", false)
    end
end)
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] and args[2] then
            ...
            TriggerEvent('cl_SendNotify', "You need to do something like /outfitadd 1 party | 1 being the slot id, party is the name of your outfit", 1)
        end
    else
        TriggerEvent('cl_SendNotify', "You are not near a wardrobe", 2)
    end
end, false)
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if actionDress == true then
        if args[1] then
            ...
            TriggerEvent('cl_SendNotify', "You need to do something like /outfitdel 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
        TriggerEvent('cl_SendNotify', "You are not near a wardrobe", 2)
    end
...
RegisterNetEvent("comet-clothing:list:outfits")
AddEventHandler("comet-clothing:list:outfits", function(data)
    TriggerServerEvent("comet-clothing:get_outfit", data.slot)
end)
