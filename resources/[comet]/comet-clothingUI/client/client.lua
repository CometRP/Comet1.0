...
RegisterNUICallback("closeNui", function(buy)
    ...
    SetEntityHeading(PlayerPedId(), false)
    toggleNuiFrame(false)
    exports["comet-clothingUI"]:closeCamera()
    TriggerEvent("comet-clothingUI:save")
    if firstspawn then
        firstspawn = false
...
RegisterNUICallback("getClothesMenu", function(data, cb)
    ...
    local menus = {
        ...
        ["clothing"] = {
            ...
            {
                name = Config.Translate[Config.Lang].Clothing.hairColor.hairColor,
                isColor = true,
                type = "hair",
                colorLabel = Config.Translate[Config.Lang].Clothing.hairColor.primaryHairColor,
                colors = getHairColors()[1],
                secondaryColorLabel = Config.Translate[Config.Lang].Clothing.hairColor.secondaryHairColor,
...
RegisterNUICallback("changeClothe", function(data, cb)
    ...
    if data.componentSettings.component_id and data.type ~= "hair" and data.type ~= "pedface" then
    ...
    elseif data.type == "hair" then
        ...
        -- print(data.currentIndex, data.currentTextureIndex)
        SetPedComponentVariation(PlayerPedId(), 2, data.currentIndex, data.currentTextureIndex, 0)
    elseif data.type == "fade" then
        local charHair = GetPedDrawableVariation(ped, 2)
        local charGender = "m"
        local Player = exports['comet-base']:FetchComponent("Player").GetPlayerData()
...
RegisterNetEvent("comet-clothingUI:opecomet-clothingUI", OpenShops)
