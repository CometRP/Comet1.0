...
function setShopBlip()
	...
	modelHash = GetHashKey("g_m_m_chigoon_01")
	RequestModel(modelHash)
	while not HasModelLoaded(modelHash) do
	 	Wait(1)
	end
	ped = CreatePed(0, modelHash , -601.87384033203, -1027.4193115234, 22.544950485229 - 1,true)
	...
	TaskStartScenarioInPlace(ped, "WORLD_HUMAN_STAND_IMPATIENT_UPRIGHT", 0, true)
				...
				created_ped11 = CreatePed(0, modelHash , 3593.9599609375, 3728.0505371094, 28.25,true)
				SetEntityHeading(created_ped11,  238.77697753906)
	
				-- Black Doctor idk
				modelHash = GetHashKey("u_m_o_tramp_01")
				RequestModel(modelHash)
				while not HasModelLoaded(modelHash) do
					 Wait(1)
				end
				...
				created_ped12 = CreatePed(0, modelHash , 246.56007385254, 369.49496459961, 104.73817443848,true)
				FreezeEntityPosition(created_ped12, true)
				SetEntityHeading(created_ped12,  79.582931518555)
				SetEntityInvincible(created_ped12, true)
				SetBlockingOfNonTemporaryEvents(created_ped12, true)
				TaskPlayAnim(created_ped12, "timetable@ron@ig_3_couch", "base", 1.0,-2.0, -1, 1, 1, true, true, true)

				modelHash = GetHashKey("g_m_y_salvagoon_02")
				RequestModel(modelHash)
				while not HasModelLoaded(modelHash) do
					 Wait(1)
				end
				created_ped12 = CreatePed(0, modelHash , -0.70153224468231, -0.45762619376183, -122.20356750488,true)
				SetEntityHeading(created_ped12,  177.11233520508)
				FreezeEntityPosition(created_ped12, true)
				SetEntityInvincible(created_ped12, true)
				SetBlockingOfNonTemporaryEvents(created_ped12, true)

				modelHash = GetHashKey("u_m_m_streetart_01")
				RequestModel(modelHash)
				while not HasModelLoaded(modelHash) do
					 Wait(1)
				end
				created_ped12 = CreatePed(0, modelHash , 689.08325195312, -1012.8077392578, 21.9,true)
				FreezeEntityPosition(created_ped12, true)
				SetEntityHeading(created_ped12, 271.76086425781)
				SetEntityInvincible(created_ped12, true)
				SetBlockingOfNonTemporaryEvents(created_ped12, true)
				TaskPlayAnim(created_ped12, "timetable@ron@ig_3_couch", "base", 1.0,-2.0, -1, 1, 1, true, true, true)
			end
...
RegisterNetEvent('racing:usb')
AddEventHandler('racing:usb', function()
    TriggerEvent("server-inventory-open", "523", "Shop");
    Wait(1000)
end)
...
AddEventHandler("open:storage", function(shop)
	local job = exports["comet-base"]:isPed("myJob")
	if job == "auto_bodies" or job == 'paleto_mech' or job == 'auto_exotics' or job == 'tuner_shop' or job == "best_buds" or job == "bean_machine" or job == "burger_shot" or job == "plane_school" or job == "gallery" then
...
AddEventHandler('pmeth:general', function()
	-- local finished = exports['ez-hud']:beginProgress(60,"Searching",false)
	-- if (finished) then  
...
RegisterNetEvent('uzi:craft')
AddEventHandler('uzi:craft', function()
	TriggerEvent("server-inventory-open", "2121", "Craft")
	Wait(1000)
end)

RegisterNetEvent('heavy:craft')
AddEventHandler('heavy:craft', function()
	TriggerEvent("server-inventory-open", "3020", "Craft")
	Wait(1000)
end)

RegisterNetEvent('tec:craft')
AddEventHandler('tec:craft', function()
	TriggerEvent("server-inventory-open", "3021", "Craft")
	Wait(1000)
end)
...
AddEventHandler('weapon:general', function()
	local weaponslicence = exports["comet-base"]:isPed("weaponslicence")
