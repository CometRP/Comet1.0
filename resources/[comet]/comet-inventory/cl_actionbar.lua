...
AddEventHandler('comet-items:SetAmmo', function(sentammoTable)
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("comet-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["comet-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["comet-inventory"]:hasEnoughOfItem(weapon,1,false) then
			...
			if selectedWeapon ~= lastWeapon and (lastWeapon == 571920712) then
				local hasItem = exports["comet-inventory"]:hasEnoughOfItem("571920712", 1, false, true, lastEquippedInfo)
				if not hasItem then
					lastEquippedInfo = {}
				else
		...
		if IsPedPlantingBomb(ped) then
			if exports["comet-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("comet-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	-- if not exports["comet-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	TriggerEvent("comet-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("comet-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("comet-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('comet-item:CheckClientAmmo')
AddEventHandler('comet-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["comet-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x722C6CA6 )		
	end
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_extended", 1, false, true) then
	...
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_taser_banana", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x12359AF7 )	
	end
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_pickaxe", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE648EB21 )	
	end
	if exports["comet-inventory"]:hasEnoughOfItem("weapon_cutlass", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x415B3660 )	
	end
	if exports["comet-inventory"]:hasEnoughOfItem("red_saber", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xF17E0A19 )	
	end
	if exports["comet-inventory"]:hasEnoughOfItem("orange_saber", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x5B27D3 )	
	end
	if exports["comet-inventory"]:hasEnoughOfItem("red_t_saber", 1, false, true) then
...
RegisterNetEvent("comet-weapons:hitPlayerWithCash", function(pTarget)
  if not lastEquippedInfo.amount then
	TriggerEvent('DoLongHudText', 'Cash could not be transferred', 2)
	return
  end

  TriggerServerEvent("comet-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
