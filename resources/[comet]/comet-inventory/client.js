...
on('watch-inventory', (src, cash, name) => {
    watch = [src, name]
    emit('chatMessage', 'SEARCH ', 2, "Player had cash in the amount of: " + cash, 'feed', false, 'Player had cash in the amount of');
});

RegisterNetEvent('comet-base:playerLoaded');
on('comet-base:playerLoaded', async (broughtData) => {
...
on('player:receiveItem', async (id, amount, generateInformation, itemdata, returnData = '{}', devItem = false) => {
    if (!(id in itemList)) {
        let plyWeight = exports['comet-inventory'].getCurrentWeight()
        if (plyWeight + itemList[id].weight > 250.0) {
            emit('DoLongHudText', 'You do not have enough room to hold the bud.', 2)
            return;
        }
        //Try to hash the ID
    ...
    if ((parseFloat(personalWeight) > maxPlayerWeight || parseFloat(personalWeight) + combined > maxPlayerWeight) && !devItem) {
        ...
        let droppedItem = { slot: 1, itemid: id, amount: amount, generateInformation: generateInformation, data: Object.assign({}, itemdata), returnData: returnData };
        cid = exports['comet-base'].FetchComponent("Player").GetPlayerData().cid;
...
//  $.post("http://comet-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('__cfx_nui:RepairSlot', (data, cb) => {
    ...
    emitNet('comet-inventory:repairSlot', slot);
    cb({});
});
...
let isCuffed = false;
RegisterNetEvent('qb-police:cuffs:state');
on('qb-police:cuffs:state', (pIsHandcuffed, pIsHandcuffedAndWalking) => {
...
on('inventory-open-request', async () => {
    ...
    const currentTarget = exports['comet-base'].FetchComponent("Player").GetEntityInFront(5.0, player)
...
function openTrunk(pEntity) {
    ...
    const vehId = GetVehicleNumberPlateText(pEntity)
    if (!vehId) {
        CloseGui();
...
on('comet-inventory:openDumpster', (pParams, pEntity, pContext) => {
    openDumpster(pEntity);
});

on('comet-inventory:openTrunk', (pParams, pEntity, pContext) => {
    openTrunk(pEntity);
});
...
RegisterNetEvent('comet-items:SetAmmo');
on('comet-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('inventory-bind', (slot) => {
    if (exports['comet-base'].isPed('dead')) {
        return;
    }
    let cid = exports['comet-base'].FetchComponent("Player").GetPlayerData().cid;
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('comet-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("comet-inventory:craftProgression", data);
    emitNet("comet-inventory:craftProgression", data);
});

RegisterNuiCallbackType('weaponCrafted');
on('__cfx_nui:weaponCrafted', (data, cb) => {
    cb("ok");
    emitNet('comet-inventory:weaponCrafted', data.item);
});
    ...
    if (canOpen === true) {
        ...
        if (targetinvName.indexOf("Shop") > -1) {
            ...
            setImmediate(async () => {
                ...
                let cop = false;
                if (exports['comet-base'].isPed('myjob') == 'police' || exports['comet-base'].isPed('myjob') == 'doc') {
                    cop = true;
                }
                await Delay(250);
...
function hasPermissionInProperty(){
    // if(exports["qb-housing"].isInsideProperty() && exports["qb-housing"].hasPermissionInProperty() == true && exports["qb-housing"].isPropertyShop()){
    //     return true
    // }
    // else
        return false 
...
function isPropertyLockdownAndIsCop(){
    // if(exports["qb-housing"].isInsideProperty() && exports["qb-housing"].isPropertyShop() && exports["qb-housing"].isCurrentPropertyLockdown()){
    //     let cop = false;
    //     if (exports['comet-base'].isPed('myjob') == 'police' || exports['comet-base'].isPed('myjob') == 'judge') {
    //         cop = true;
    //     }
    //     if (cop){
    //         return true;
...
on('inventory-open-container', async (inventoryId, slots, weight) => {
    ...
    const cid = exports['comet-base'].FetchComponent("Player").GetPlayerData().cid;
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('comet-inventory:server:weightChange', cid, weight);
...
// let tradingIn = false;
// on('comet-inventory:sionisReceipt', async ([pClass]) => {
//     if (tradingIn) {
//         return;
//     }
//     tradingIn = true;
...
//     // }
//     emit('doAnim', 'cokecut');
//     // const finished = await new Promise((r) => exports['comet-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
//     ClearPedTasks(PlayerPedId());
//     // if (finished !== 100) {
//     //     return;
//     // }
...
//             return;
//         }
//         const qty = Math.min(exports['comet-inventory'].getQuantity(mat, true), 50000);
...
//     TriggerServerEvent('comet-inventory:sionisTaxedMats', taxedMats);
//     canOpen = true;
//     tradingIn = false;
// });

// on('comet-inventory:sionisReceiptTradeIn', async () => {
//     const items = exports['comet-inventory'].getItemsOfType('refinedmaterialreceipt', 1000, true);
//     if (items.length === 0) {
//         return;
//     }
//     // const [hasCraftAccess] = await exports['comet-base'].FetchComponent("Callback").Execute('qb-business:hasPermission', 'sionis', 'craft_access');
...
// on('comet-inventory:sionisCollectTaxes', () => {
//     const ids = exports['qb-config'].GetMiscConfig('sionis.pickup.ids');
//     const cid = exports['comet-base'].FetchComponent("Player").GetPlayerData().cid;
//     if (ids.indexOf(cid) === -1) {
//         return;
//     }
//     TriggerServerEvent('comet-inventory:sionisCollectTaxes')
// });
...
// on('comet-inventory:itemUsed', async (pItem, pInfo, pInv, pSlot) => {
    // QBCORE NOPIXEL
    ...
    // }
    // // COME BACK
    // const prompt = await exports['qb-interface'].OpenInputMenu(
    //     [
    //         {
    //             label: 'Action',
    //             name: 'action',
    ...
    //         const qty = Math.min(exports['comet-inventory'].getQuantity(key, true), 50000);
    //         if (!qty) {
    //             continue;
    //         }
    //         if (!meta[key]) {
    ...
    //     TriggerServerEvent('comet-inventory:matBagUpdate', meta.id, meta);
    //     return;
    // }
...
// tradingIn = false;
// on('qb-cia:depositMaterials', async () => {
//     if (tradingIn) return;
//     tradingIn = true;
//     emit('doAnim', 'cokecut');
//     const finished = await new Promise((r) => exports['comet-taskbar'].taskBar(10000, 'Gathering materials...', true, true, null, false, r));
...
//     const qty = Math.min(exports['comet-inventory'].getQuantity('cia_weapon_material', true), 50000);
//     if (qty < 1) return;
//     emit('inventory:removeItem', 'cia_weapon_material', qty);
//     TriggerServerEvent('comet-inventory:depositCiaMaterials', qty);
// });

// on('qb-cia:checkMaterials', async () => {
//     if (tradingIn) return;
//     TriggerServerEvent('comet-inventory:checkCiaMaterials');
// });

// on('qb-cia:getGear', async () => {
//     let response = 1;
//     while (response) {
//         await new Promise(r => setTimeout(r, 250));
//         response = await exports['qb-interface'].OpenContextMenu([
//             {
//                 key: { item: '-820634585' },
//                 title: 'Taser',
//             },
...
//         ]);
//         if (!response) break;
//         TriggerServerEvent('comet-inventory:ciaGetGear', response.item);
//     }
// });

// AddEventHandler("qb-cia:getGear", function()
  
// end)

// AddEventHandler("qb-cia:checkMaterials", function()
