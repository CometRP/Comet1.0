...
function checkForAlternative(pItem)
	if pItem and exports["comet-inventory"]:getQuantity(pItem["alternativeItem"]) > 0 then
	...
	for i = 1, #w do
		if exports["comet-inventory"]:getQuantity(w[i]["id"]) > 0 then
			...
			elseif w[i]["type"] == 4 and not sheathed then
				TriggerEvent("comet-inventory:attachmentsToggle", true, w[i]["id"])
				sheathed = true
...
function DeleteAttached()
	TriggerEvent("comet-inventory:attachmentsToggle", false)
	for i = 1, #ag do
		DeleteEntity(ag[i])
	end
	for i = 1, #ad do
