...
function DeleteOld() {
    ...
    for (let i = saveList.lengh - 1; i > 0; i--) {
        ...
        if (itemList[ItemID].fullyDegrades) {
            exports["oxmysql"].execute(`DELETE FROM user_inventory2 WHERE item_id = '${ItemID}' AND ${DeleteTime} > creationDate`);
        } else {
...
function CleanDroppedInventories() {
    exports["oxmysql"].execute(`DELETE FROM user_inventory2 WHERE dropped = '1'`)
    exports["oxmysql"].execute(`DELETE FROM user_inventory2 WHERE name LIKE '%_trash'`)
    DeleteOld()
}
