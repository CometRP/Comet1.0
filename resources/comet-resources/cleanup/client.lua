...
AddEventHandler("cleanup:enableCleanup", function(pEnabled)
  skipCleanupChecks = not pEnabled
end)

AddEventHandler("cleanup:addBlacklistedProp", function(pProp)
...
local bypassObjects = {}
AddEventHandler('objects:objectCreated', function(pObject, pHandle)
    bypassObjects[pHandle] = true
end)

AddEventHandler('objects:objectDeleted', function(pObject, pHandle)
    bypassObjects[pHandle] = nil
end)

AddEventHandler('objects:objectRecreating', function(pObject, pHandle)
    bypassObjects[pHandle] = nil
end)

AddEventHandler('objects:objectRecreated', function(pObject, pHandle)
    bypassObjects[pHandle] = true
end)
    ...
    while true do
        ...
        for _,handle in ipairs(propList) do
            ...
            if not success then
                ...
                -- exports["comet-base"]:FetchComponent("Sync").DeleteEntity(handle)
                exports['comet-base']:SyncedExecution('DeleteEntity', handle)
                goto continue 
            end
...
RegisterNetEvent('cleanup:clearAreaOfEverything')
AddEventHandler('cleanup:clearAreaOfEverything', function (vectors, radius)
    ClearAreaOfEverything(vectors.x, vectors.y, vectors.z, radius, false, false, false, false)
end)

RegisterNetEvent('cleanup:clearAreaOfObjects')
AddEventHandler('cleanup:clearAreaOfObjects', function (vectors, radius)
    ClearAreaOfObjects(vectors.x, vectors.y, vectors.z, radius)
end)

RegisterNetEvent('cleanup:clearAreaOfPeds')
AddEventHandler('cleanup:clearAreaOfPeds', function (vectors, radius)
    ClearAreaOfPeds(vectors.x, vectors.y, vectors.z, radius)
end)

RegisterNetEvent('cleanup:clearAreaOfVehicles')
AddEventHandler('cleanup:clearAreaOfVehicles', function (vectors, radius)
