    ...
    exports.oxmysql:execute("SELECT * FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            if result[1].model == nil then
                TriggerClientEvent("cl_SendNotify", src, "Can not use.",2)
                return
            end
            ...
            TriggerClientEvent("cl_SendNotify", src, "No outfit on slot " .. slot,2)
            return
        end
	end)
end)
    ...
    exports.oxmysql:execute("SELECT slot FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            exports.oxmysql:execute("INSERT INTO character_outfits ("..cols..") VALUES ("..vals..")", values, function()
                TriggerClientEvent("cl_SendNotify", src, name .. " stored in slot " .. slot,1)
            end)
        end
	end)
end)
...
AddEventHandler("comet-clothing:remove_outfit",function(slot)
    ...
    TriggerClientEvent("cl_SendNotify", src,"Removed slot " .. slot .. ".",1)
end)
...
AddEventHandler("comet-clothing:list_outfits",function()
    ...
    exports.oxmysql:execute("SELECT slot, name FROM character_outfits WHERE cid = @cid", {['cid'] = cid}, function(skincheck)  
        for i = 1, #skincheck do
            TriggerClientEvent('comet-context:sendMenu', src, {
                {
                    id = skincheck[i].slot,
...
AddEventHandler("clothing:checkIfNew", function()
    ...
    exports.oxmysql:execute("SELECT count(rank) whitelist FROM jobs_whitelist WHERE cid = @cid LIMIT 1", {
        ...
        exports.oxmysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            TriggerClientEvent("comet-clothing:inService",src,isService)
    	end)
    end)
end)
...
AddEventHandler("clothing:checkMoney", function(askingPrice)
    local src = source
    local target = Player.GetUser(src)
