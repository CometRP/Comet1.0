...
function Components.Database.CreateNewCharacter(self, src, data2, hexid, ph, callback)
    local hexid = hexid
    callback = callback and callback or function() return end
...
function Components.Database.PhoneNumberExists(self, src, phone_number, callback)
...
function Components.Database.FetchCharacterData(self, user, callback)
    callback = callback and callback or function() return end
...
function Components.Database.DeleteCharacter(self, user, id, callback)
    callback = callback and callback or function() return end
...
function Components.Database.UpdateCharacterStressLevel(self, user, characterId, newLevel, callback)
    callback = callback and callback or function() return end
    if not user then callback(false,true) return end
...
function Components.Database.UpdateCharacterDirtyMoney(self, user, characterId, amount, callback)
callback = callback and callback or function() return end
    if not user then callback(false,true) return end
    if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function Components.Database.UpdateCharacterMoney(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function Components.Database.UpdateCharacterBank(self, user, characterId, amount, callback)
    callback = callback and callback or function() return end
        if not user then callback(false,true) return end
        if not characterId or type(characterId) ~= "number" then callback(false, true) return end
...
function Components.Database.UpdateControls(self, src, controlsTable, callback)
    callback = callback and callback or function() return end
    local user = Components.Player:GetUser(src)
    if not user then callback(false,true) return end
...
function Components.Database.GetControls(self, src, callback)
    callback = callback and callback or function() return end
    local user = Components.Player:GetUser(src)
    Citizen.Wait(3000)
    if not user then callback(false, true) return end
...
function Components.Database.UpdateSettings(self, src, settingsTable, callback)
    callback = callback and callback or function() return end
    local user = Components.Player:GetUser(src)
    if not user then callback(false,true) return end
...
function Components.Database.GetSettings(self, src, callback)
    callback = callback and callback or function() return end
    local user = Components.Player:GetUser(src)
    if not user then callback(false, true) return end
