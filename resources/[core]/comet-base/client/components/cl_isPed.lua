...
function Components.isPed.GlobalObject(object)
...
exports("GlobalObject", Components.isPed.GlobalObject)
local pedsused = {}



function Components.isPed.GetClosestNPC()
    ...
    repeat
        ...
        if Components.isPed.canPedBeUsed(ped) and distance < 5.0 and (distanceFrom == nil or distance < distanceFrom) then
...
exports("GetClosestNPC", Components.isPed.GetClosestNPC)

function Components.isPed.canPedBeUsed(ped)
    if ped == nil then
        return false
    end
...
exports("canPedBeUsed", Components.isPed.canPedBeUsed)

function Components.isPed.IsPedNearCoords(x,y,z)
    local handle, ped = FindFirstPed()
...
exports("IsPedNearCoords", Components.isPed.IsPedNearCoords)


function Components.isPed.GroupRank(groupid)
  local rank = 0
  for i=1, #passes do
...
exports("GroupRank", Components.isPed.GroupRank)

function Components.isPed.isPed(checkType)
...
exports("isPed", Components.isPed.isPed)
...
AddEventHandler("ped:intrunk", function(trunksent)
  intrunk = trunksent
  exports['comet-flags']:SetPedFlag(PlayerPedId(), 'isInTrunk', intrunk)
end)
...
function Components.isPed.retreiveBusinesses()
...
exports("retreiveBusinesses", Components.isPed.retreiveBusinesses)
...
AddEventHandler("retreiveBusinesses", function()
    TriggerServerEvent("showbusinesses",Components.isPed.retreiveBusinesses())
end)
...
AddEventHandler("robbing",function(passedR)
    robbing = passedR
    exports['comet-flags']:SetPedFlag(PlayerPedId(), 'isRobbing', intrunk)
end)
...
AddEventHandler('pd:deathcheck', function()
	dead = not dead
  exports['comet-flags']:SetPedFlag(PlayerPedId(), 'isDead', dead)
end)
...
AddEventHandler('police:currentHandCuffedState', function(handCuffed,handCuffedWalking)
  ...
  exports['comet-flags']:SetPedFlag(PlayerPedId(), 'isCuffed', handCuffed or handCuffedWalking)
end)
...
RegisterNetEvent('comet-base:addedMoney')
AddEventHandler('comet-base:addedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)

RegisterNetEvent('comet-base:removedMoney')
AddEventHandler('comet-base:removedMoney', function(blah,amount)
  cash = math.ceil(amount)
end)
...
AddEventHandler('client:pass', function(passType,args)
    ...
    if rank < 1 then
      if passType == "strip_club" then
        TriggerEvent("cl_SendNotify","Hm, I wonder if I can join this club?",1)
      else
        TriggerEvent("cl_SendNotify","You do not have permission to do this.",2)
      end
      return
    end
    if passType == "golfcourse" then
      TriggerServerEvent("GolfPay")
      TriggerEvent("cl_SendNotify","Press E to swing, A-D to rotate, Y to swap club.",1)
    end
    if passType == "gym" then
      TriggerEvent("doworkout")
      TriggerEvent("cl_SendNotify","Starting, hold still..",1)
    end
    if passType == "strip_club" then
      TriggerEvent("dostripstress")
      TriggerEvent("cl_SendNotify","Ahhh, this is relaxing.",1)
    end    
...
RegisterNetEvent('comet-phone:fix-dead')
AddEventHandler('comet-phone:fix-dead', function()
  dead = false
end)