...
$(document).ready(function () {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://comet-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.response == 'openGui') {
            ...
            $('#app').fadeIn();
            $('#containers-wrapper').slideDown(0);
        } else if (item.response == 'closeGui') {
            $('#app').fadeOut(0);
            $('#containers-wrapper').fadeOut(0);
            ...
            if (itemList[item.id]) {
                $.post(
                    'https://comet-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function UseBar(itemid, text, amount) {
    ...
    htmlstring =
        " <div class='item4' > <div class='UseBarHead'> " +
        text +
        ' ' +
        amount +
        "x  </div> <div class='itemname4'> " +
        name +
        " </div> <img src='icons/" +
        image +
        "' class='itemimage'>  </div>";
...
function ToggleBar(toggle, boundItems, boundItemsAmmo) {
    if (toggle) {
        ...
        if (boundItems[1]) {
            ...
            htmlstring =
                "</div> <div class='item4' > <div class='itemname4'> " +
                name +
                " </div> <img src='icons/" +
                image +
                "' class='itemimage'>  </div>";
            ...
            htmlstring =
                "</div> <div class='item4' > <div class='itemname4'> unbound </div> <img src='icons/empty.png' class='itemimage'>  </div>";
        }
        ...
        $('#ActionBar').fadeIn(1000);
    } else {
        $('#ActionBar').fadeOut(1000);
    }
}
...
function invStack(
    ...
    $.post('https://comet-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://comet-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://comet-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft;
    let amount = moveAmount;
    $.post('https://comet-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, targetInvWeight = 0) {
    ...
    } else if (targetinvName.indexOf('Container') > -1) {
    ...
    } else if (targetinvName.indexOf('gem_table_counter') > -1) {
        secondaryMaxWeight = 100.0;
        slotLimitTarget = 10;
        displayName = 'Gem Table';
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://comet-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function UpdateSetWeights(secondaryName) {
    ...
    if (!$('#playerWeight').html() || $('#secondaryInvName').attr('title') !== TargetInventoryName) {
        $('#wrapPersonalWeight').html(
                ...
                "%;'class='weight-current'> <span class='weight-max'>" 
        );

            $('#wrapPersonalWeight3').html(
                    "<div id='playerWeight' style='width:" +
                    (personalWeight / personalMaxWeight) * 100 +
                    "'class='weight-current'> " +
                    personalWeight.toFixed(1) +
                    "/" +
                    personalMaxWeight.toFixed(1) +
                    '</span></div>',
            );

            
            $('#wrapSecondaryWeight3').html(
                "<div id='playerWeight' style='width:" +
                (secondaryWeight / secondaryMaxWeight) * 100 +
                "'class='weight-current'> " +
                secondaryWeight.toFixed(1) +
                "/" +
                secondaryMaxWeight.toFixed(1) +
                '</span></div>',
        );

        $('#wrapPersonalWeight2').html(
            "<h3 title='" + PlayerInventoryName + "'>" + "Inventory"
        );
        
        if (!secondaryName && $('#secondaryInvName').attr('title') !== TargetInventoryName) secondaryName = 'Ground';

        let secondaryHeader = "<h3 id='secondaryInvName' title='" + "'>" + "" + '</h3>';
        let secondaryHeader2 = "<h3 id='secondaryInvName' title='" + TargetInventoryName + "'>" + secondaryName + '</h3>';

        $('#wrapSecondaryWeight2').html(
            "<h3 title='" + secondaryHeader2
        );
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            $.post('https://comet-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://comet-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://comet-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://comet-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://comet-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://comet-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://comet-inventory/ServerCloseInventory',
        JSON.stringify({
...
function useitem() {
    ...
    if (inventoryUsedName == PlayerInventoryName) {
        ...
        $.post('https://comet-inventory/invuse', JSON.stringify(arr));
