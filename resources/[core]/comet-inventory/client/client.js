...
on('player:receiveItem', async(id, amount, generateInformation, itemdata, returnData = '{}', devItem = false) => {
    ...
    if (resource === null) {
        ...
        if (parseFloat(personalWeight) > 150 || parseFloat(personalWeight) + combined > 150) {
            emit('cl_SendNotify', 'Failed to give ' + id + ' because you were overweight!', 2);
        ...
        emitNet("comet-logs:cheaterDetected", "Inventory", "Event Triggered")
        // exports['comet-notify'].SendNotify("Bro dont cheat :kekw:", 2)
        // emit('cl_SendNotify', 'Bro dont cheat :kekw:!', 2);
    }
...
function findSlot(ItemIdToCheck, amount, nonStacking) {
    ...
    if (foundslot == 0) {
        emit('cl_SendNotify', 'Failed to give ' + ItemIdToCheck + ' because you were full!', 2);
    }
        ...
        if (GetVehicleDoorLockStatus(vehicleFound) == 2 && distanceRear < 1.5) {
            ...
            if (distanceRear > 1.5) {
                ...
                if (licensePlate != null) {
                    ...
                    if (GetVehicleClass(vehicleFound) == "12") {
...
RegisterNetEvent('closeInventoryGui2');
on('closeInventoryGui2', () => {
    CloseGui2();
    //closegui
});
...
on('__cfx_nui:stack', (data, cb) => {
    ...
    TriggerServerEvent('InteractSound_SV:PlayOnSource', 'inv', 0.5)
});
...
async function CloseGui2(pIsItemUsed = false) {
    if (!pIsItemUsed) {
    }
    SendNuiMessage(JSON.stringify({ response: 'closeGui' }));
    SetCustomNuiFocus(false, false);
