    ...
    let dateNow = Date.now()
        for (let i = savelist.lengh - 1; i >= 0; i++) {
            ...
            if (itemList[itemID].fullyDegrades) {
                db(`DELETE FROM user_inventory WHERE item_id = '${ItemID}' AND ${DeleteTime} > creationDate`);
...
function CleanDroppedInventories() {
    db(`DELETE FROM user_inventory WHERE dropped = '1'`);
    db(`DELETE FROM user_inventory WHERE name = 'trash-1'`);
    DeleteOld()
}
