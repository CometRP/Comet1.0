...
onNet("server-update-item", async (player, itemidsent, slot, data) => {
    ...
    let string = `UPDATE user_inventory SET information='${data}' WHERE item_id='${itemidsent}' and name='${playerinvname}' and slot='${slot}'`
...
function functionRemoveAny(player, itemidsent, amount, openedInv) {
    let src = source
    let playerinvname = 'ply-' + player
    let string = `DELETE FROM user_inventory WHERE name='${playerinvname}' and item_id='${itemidsent}' LIMIT ${amount}`
...
onNet("server-request-update", async (player) => {
    let src = source
    let playerinvname = 'ply-' + player
    let string = `SELECT count(item_id) as amount, id, item_id, name, information, slot, dropped FROM user_inventory WHERE name = 'ply-${player}' group by item_id`;
...
//     let amount = 4000000
//     exports["oxmysql"].execute(`UPDATE user_inventory set creationDate = ${amount} WHERE id = ${itemID}`, {}, function() {});
// });
...
onNet("inventory:degItem", async (LastUsedItem, percent, LastUsedItemId, cid) => {
    ...
    exports["oxmysql"].execute(`UPDATE user_inventory SET creationDate = creationDate - ${amount} WHERE id = ${LastUsedItem} AND item_id = '${LastUsedItemId}' AND name = '${playerInventory}'`, {}, function() {});
});
...
onNet("server-request-update-src", async (player, src) => {
    ...
    let playerinvname = 'ply-' + player
    let string = `SELECT count(item_id) as amount, item_id, id, name, information, slot, dropped, creationDate, MIN(creationDate) as creationDate, quality FROM user_inventory WHERE name = '${playerinvname}' group by item_id`; // slot
    exports["oxmysql"].execute(string, {}, function(inventory) {
        emitNet("inventory-update-player", src, [inventory, 0, playerinvname]);
    });
});
...
onNet("server-inventory-give", async (player, itemid, slot, amount, generateInformation, itemdata, openedInv) => {
    ...
    let query = `INSERT INTO user_inventory (name,item_id,information,slot,creationDate) VALUES ${values};`
...
onNet("server-inventory-refresh", async (player, sauce) => {
    ...
    let string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped,quality, creationDate FROM user_inventory where name= 'ply-${player}' group by slot`;
    exports["oxmysql"].execute(string, {}, function(inventory) {
        if (!inventory) {} else {
...
onNet("server-inventory-open", async (coords, player, secondInventory, targetName, itemToDropArray, sauce) => {
    ...
    //    emitNet('current-items', src, invArray)
        ...
        if (secondInventory == "1") {
            ...
            let string = `SELECT count(item_id) as amount, id, name, item_id, information, slot, dropped, creationDate FROM user_inventory WHERE name = '${targetinvname}' group by slot`;
            ...
            for (let Key in itemToDropArray) {
                for (let i = 0; i < itemToDropArray[Key].length; i++) {
                    ...
                    db(`UPDATE user_inventory SET slot='${i+1}', name='${NewDroppedName}', dropped='1' WHERE name='${Key}' and slot='${objectToDrop.faultySlot}' and item_id='${objectToDrop.faultyItem}' `);
                }
            }
            ...
            var shopAmount = 19;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "1020") {
            var targetinvname = targetName;
            var shopArray = MethStore();
            var shopAmount = 3;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
            ...
            var shopAmount = 1;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "523") {
            var targetinvname = targetName;
            var shopArray = Racing();
            var shopAmount = 2;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
            ...
            var shopAmount = 7;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "2121") {
            var targetinvname = targetName;
            var shopArray = StupidBanFreeThem();
            var shopAmount = 1;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "3020") {
            var targetinvname = targetName;
            var shopArray = HeavyCraft();
            var shopAmount = 1;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "3021") {
            var targetinvname = targetName;
            var shopArray = TecCraft();
            var shopAmount = 1;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "3421") {
            var targetinvname = targetName;
            var shopArray = APCraft();
            var shopAmount = 2;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
        } else if (secondInventory == "3452") {
            var targetinvname = targetName;
            var shopArray = BookClubIsGay();
            var shopAmount = 2;
            emitNet("inventory-open-target", src, [invArray, arrayCount, playerinvname, shopArray, shopAmount, targetinvname, 500, false]);
...
onNet("server-inventory-close", async (player, targetInventoryName) => {
    ...
    if (targetInventoryName.indexOf("Drop") > -1 && DroppedInventories[targetInventoryName]) {
        if (DroppedInventories[targetInventoryName].used === false) {
            ...
            let string = `SELECT count(item_id) as amount, item_id, name, information, slot, dropped FROM user_inventory WHERE name='${targetInventoryName}' group by item_id `;
            exports["oxmysql"].execute(string, {}, function(result) {
...
onNet("server-inventory-remove-slot", async (player, itemidsent, amount, slot) => {
    ...
    db(`DELETE FROM user_inventory WHERE name='${playerinvname}' and item_id='${itemidsent}' and slot='${slot}' LIMIT ${amount}`);
});
...
onNet("server-ragdoll-items", async (player) => {
    ...
    db(`UPDATE user_inventory SET name='${newInventoryName}', WHERE name='${currInventoryName}' and dropped=0 and item_id="mobilephone" `);
    db(`UPDATE user_inventory SET name='${newInventoryName}', WHERE name='${currInventoryName}' and dropped=0 and item_id="idcard" `);
    await db(`DELETE FROM user_inventory WHERE name='${currInventoryName}'`);
    db(`UPDATE user_inventory SET name='${currInventoryName}', WHERE name='${newInventoryName}' and dropped=0`);
});
    ...
    if (purchase) {
        ...
        if (!PlayerStore) {
            for (let i = 0; i < parseInt(amount); i++) {
                ...
                db(`INSERT INTO user_inventory (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetName}','${info}','${targetslot}','${creationDate}' );`);
            }
        } else if (crafting) {
            ...
            if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
                ...
                db(`INSERT INTO user_inventory SET slot='${targetslot}', name='${targetName}', dropped='1' WHERE slot='${startslot}' AND name='${startname}'`);

            } else {
                db(`UPDATE user_inventory SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE slot='${startslot}' and name='${startname}'`);
...
onNet("server-inventory-stack", async (player, data, coords) => {
    ...
    if (purchase) {
        ...
        let string = `SELECT item_id, id FROM user_inventory WHERE slot='${originSlot}' and name='${originInventory}' LIMIT ${moveAmount}`;
        ...
        exports["oxmysql"].execute(string, {}, function(result) {
            ...
            if (targetName.indexOf("Drop") > -1 || targetName.indexOf("hidden") > -1) {
                db(`UPDATE user_inventory SET slot='${targetslot}', name='${targetName}', dropped='1' WHERE id IN (${itemids})`);

            } else {
                db(`UPDATE user_inventory SET slot='${targetslot}', name='${targetName}', dropped='0' WHERE id IN (${itemids})`);
            }
        });
    }
});
...
onNet("server-inventory-swap", (player, data, coords) => {
    ...
    let string = `SELECT id FROM user_inventory WHERE slot='${targetslot}' AND name='${targetname}'`;
    exports["oxmysql"].execute(string, {}, function(startid) {
        ...
        let string = false;
        if (targetname.indexOf("Drop") > -1 || targetname.indexOf("hidden") > -1) {
            string = `UPDATE user_inventory SET slot='${targetslot}', name ='${targetname}', dropped='1' WHERE slot='${startslot}' AND name='${startname}'`;
        } else {
            string = `UPDATE user_inventory SET slot='${targetslot}', name ='${targetname}', dropped='0' WHERE slot='${startslot}' AND name='${startname}'`;
        }

        exports["oxmysql"].execute(string, {}, function(inventory) {
            if (startname.indexOf("Drop") > -1 || startname.indexOf("hidden") > -1) {
                db(`UPDATE user_inventory SET slot='${startslot}', name='${startname}', dropped='1' WHERE id IN (${itemids})`);
            } else {
                db(`UPDATE user_inventory SET slot='${startslot}', name='${startname}', dropped='0' WHERE id IN (${itemids})`);
            }
        });
    });
});
...
function deleteHidden() {
     db(`DELETE FROM user_inventory WHERE name like '%Hidden%' OR name like '%trash%'`)
 }
 ...
 onNet("inv:playerSpawned", () => {
     db(`DELETE FROM user_inventory WHERE name like '%Drop%' OR name like '%Hidden%' OR name like '%trash-1%'`)
     db(`DELETE FROM user_inventory WHERE quality like '0'`)
     console.log("[comet-inventory] Drops & broken items were deleted.")
 }); 
