...
CloseBankContainer = function() {
    $.post('https://comet-financials/Financials/Close', JSON.stringify({IsAtBank: IsAtBank}));
...
SetCashBalance = function() {
    $.post('https://comet-financials/Financials/GetCash', JSON.stringify({}), function(Amount) {
...
RefreshBankingData = function() {
    $.post('https://comet-financials/Financials/GetAccounts', JSON.stringify({}), function(AccountData) {
        ...
        $('.bank-banking-data').hide(0, function() {
            ...
            setTimeout(function() {
                if (CurrentContextType == 'Withdraw') {
                    $.post('https://comet-financials/Financials/Withdraw', JSON.stringify({Amount: Math.floor(Amount), Reason: Reason, AccountId: CurrentContextData.AccountId}), function(IsDone) {
                        if (IsDone) {
                            CloseBankContext();
                            setTimeout(function() {
                                RefreshBankingData();
                            ...
                            $.post('https://comet-financials/Financials/ErrorSound');
                            CloseBankContext();
                        }
                    });
                } else if (CurrentContextType == 'Deposit') {
                    $.post('https://comet-financials/Financials/Deposit', JSON.stringify({Amount: Math.floor(Amount), Reason: Reason, AccountId: CurrentContextData.AccountId}), function(IsDone) {
                        if (IsDone) {
                            CloseBankContext();
                            setTimeout(function() {
                                RefreshBankingData();
                    ...
                    $.post('https://comet-financials/Financials/Transfer', JSON.stringify({Amount: Math.floor(Amount), Reason: Reason, ToAccountId: TransferNumber.toString(), AccountId: CurrentContextData.AccountId}), function(IsDone) {
                        if (IsDone) {
                            CloseBankContext();
                            setTimeout(function() {
                                RefreshBankingData();
