...
CloseMobileData = function() {
    $('.tooltip-css').remove();
    $.post('https://comet-mdw/MDW/Close', JSON.stringify({}));
...
DoMdwStartup = async function(CitizenId, IsPublic) {
    ...
    if (!MdwData.UserData.IsPublic) {
        $.post('https://comet-mdw/MDW/Main/GetUser', JSON.stringify({
            CitizenId: CitizenId,
        }), function(Data) {
...
DoMdwPicker = function(Type, OnClick, ExtraData) {
    if (Type == 'Profiles') {
        ...
        $.post("https://comet-mdw/MDW/Profiles/GetProfiles", JSON.stringify({}), function(Result){
