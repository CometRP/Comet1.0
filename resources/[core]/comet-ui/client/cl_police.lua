...
RegisterNetEvent('comet-ui/client/send-emergency-alert', function(AlertData, ForBoth, SendLocation)
    ...
    if Player.Job ~= nil and (Player.Job.Name == 'police' or (ForBoth and Player.Job.Name == 'ems')) and Player.Job.Duty then
        ...
        if AlertData.AlertType == 'alert-panic' then
            EventsModule.TriggerServer('comet-ui/server/play-sound-on-entity', '10-1314', GetPlayerServerId(PlayerId()), 3000, 15.0, nil, true)
        elseif AlertData.AlertType == 'alert-red' then
            EventsModule.TriggerServer('comet-ui/server/play-sound-on-entity', 'HighPrioCrime', GetPlayerServerId(PlayerId()), 3000, 15.0, nil, true)
        else
...
RegisterNetEvent('comet-ui/client/open-alerts-menu', function(OnPress)
...
RegisterNetEvent('comet-ui/client/ui-reset', function()
    exports['comet-ui']:SendUIMessage("Police", "CloseDispatch")
end)

-- [ Functions ] --

AddInitialize(function()
    KeybindsModule.Add("alertMenu", 'Services', 'Dispatch Menu', '', false, 'comet-ui/client/open-alerts-menu')
end)
...
RegisterNUICallback('Police/SetWaypoint', function(Data, Cb)
    ...
    exports['comet-ui']:Notify("police-waypoint", "Waypoint set.", "primary")
    Cb('Ok')
end)

RegisterNUICallback('Police/GetOnDutyPeople', function(Data, Cb)
    local Data = CallbackModule.SendCallback('comet-ui/server/police/get-duty-people')
    Cb(Data)
end)

RegisterNUICallback('Police/GetDispatchData', function(Data, Cb)
    local Data = CallbackModule.SendCallback('comet-ui/server/police/get-dispatch-data')
    local DispatchData = {
