...
RegisterNetEvent('comet-ui/client/ui-reset', function()
    ...
    exports['comet-inventory']:SetBusyState(false)
...
RegisterCommand('testskill', function(source, args, RawCommand)
    local Outcome = exports['comet-ui']:StartSkillTest(3, { 7, 10 }, { 1000, 1500 }, false)
    if Outcome then
        print('Success')
    else
        print('Failed')
...
function StartSkillTest(NeededStreak, SkillGap, ReqSpeed, Reverse)
    ...
    RequestStreamedTextureDict("comet_sprites")
...
function BuildSkillTest(NeededStreak, SkillGap, ReqSpeed, Reverse, Callback)
    ...
    exports['comet-inventory']:SetBusyState(true)
    RequestStreamedTextureDict("comet_sprites")
    while not HasStreamedTextureDictLoaded("comet_sprites") do
        Citizen.Wait(0)
    end
    ...
    while DoingTask do
        ...
        DrawSprite("comet_sprites", "circle_128", OriginX, OriginY + (SpriteH / 3), SpriteW, SpriteH, 0, Colors.BgColor.Red, Colors.BgColor.Green, Colors.BgColor.Blue, 255)
        SetScriptGfxDrawOrder(9)

        DrawSprite("comet_sprites", SkillToSprite(SkillGapSent), OriginX, OriginY + (SpriteH / 3.0), SpriteW, SpriteH, TargetRotation, Colors.HudColor.Red, Colors.HudColor.Green, Colors.HudColor.Blue, 255)
        SetScriptGfxDrawOrder(8)

        DrawSprite("comet_sprites", "cursor_128", OriginX, OriginY + (SpriteH / 3), SpriteW, SpriteH, CursorRot, Colors.CursorColor.Red, Colors.CursorColor.Green, Colors.CursorColor.Blue, 255)
    ...
    Citizen.SetTimeout(500, function()
        if not DoingTask then
            SetStreamedTextureDictAsNoLongerNeeded('comet_sprites')
        end
    end)
