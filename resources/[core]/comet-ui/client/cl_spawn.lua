...
RegisterNetEvent('comet-housing/client/sync-house-data', function(Name, HouseData, RefreshInterior)
...
RegisterNetEvent('comet-spawn/client/open-spawn-selector', function()
    TriggerEvent('comet-weathersync/client/set-default-weather', 17)
    ...
    Citizen.SetTimeout(750, function()
        ...
        -- Own Houses
        local Houses = exports['comet-housing']:GetHouseConfig()
        ...
        -- Keyholder Houses
        local Result = CallbackModule.SendCallback('comet-houses/server/get-houses-keyholder', PlayerData.CitizenId)
...
RegisterNUICallback('Spawn/SelectSpawn', function(Data, Cb)
    ...
    if Data.Id == 'last_location' then
        local PlayerData = exports['comet-base']:FetchModule('Player').GetPlayerData()
        if PlayerData.Position ~= nil then
            SpawnData = {
                Type = 'Location',
                Coords = {
    ...
    if not LocalPlayer.state.LoggedIn then
        TriggerEvent('comet-base/client/on-login')
    end
    ...
    local PlayerData = exports['comet-base']:FetchModule('Player').GetPlayerData()
    while PlayerData == nil do
        PlayerData = exports['comet-base']:FetchModule('Player').GetPlayerData()
	Wait(100)
    end
		
    if SpawnData.Type == 'Location' then
        EventsModule.TriggerServer("comet-base/server/bucketmanager/set-routing-bucket", 0)
    ...
    elseif SpawnData.Type == 'Apartment' then
        ...
        Citizen.SetTimeout(500, function()
            FreezeEntityPosition(PlayerPedId(), false)
            Citizen.Wait(2500)
            TriggerEvent('comet-apartment/client/spawn-apartment', false)
        end)
    elseif SpawnData.Type == 'Jail' then
        ...
        Citizen.SetTimeout(500, function()
            FreezeEntityPosition(PlayerPedId(), false)
            Citizen.Wait(1250)
            TriggerEvent('comet-police/client/enter-jail', PlayerData.MetaData['Jail'], 0, true)
        end)
    elseif SpawnData.Type == 'House' then
        EventsModule.TriggerServer("comet-base/server/bucketmanager/set-routing-bucket", 0)
        local HouseData = exports['comet-housing']:GetHouseById(string.sub(Data.Id, 7, #Data.Id))
        ...
        Citizen.SetTimeout(2500, function()
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('comet-housing/client/enter-house', string.sub(Data.Id, 7, #Data.Id))
        end)
    end
