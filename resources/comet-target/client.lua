			...
			if not success then
				...
				for k, zone in pairs(Zones) do
					...
					if Config.DrawSprite then
						local ppDist = #(GetEntityCoords(PlayerPedId()) - zone.center)
						local pDist = #(coords - zone.center)
						if ppDist < (zone.targetoptions.drawDistance or Config.DrawDistance) then
							listSprite[k] = zone
							listSprite[k].distanceSprite = 30
							if pDist < 2.5 then
								listSprite[k].distanceSprite = 60
							end
							if pDist < 2.0 then
								listSprite[k].distanceSprite = 120
							end
							if pDist < 1.5 then
								listSprite[k].distanceSprite = 200
							end
							if pDist < 1.0 then
								listSprite[k].distanceSprite = 255
							end
						else
							listSprite[k] = nil
						end
	...
	CreateThread(function()
		...
		elseif data.event then
			...
			elseif data.type == "custom" then
				TriggerServerEvent('comet-base:callCommand', data.event, data)
			else
				TriggerEvent(data.event, data)
			end
		else
